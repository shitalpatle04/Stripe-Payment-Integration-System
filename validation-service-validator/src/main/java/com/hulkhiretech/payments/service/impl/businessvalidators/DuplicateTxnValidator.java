...
public class DuplicateTxnValidator implements BusinessValidator {
	...
	public void validate(PaymentRequest paymentRequest) {
		log.info("Validating paymentRequest: {}", paymentRequest);
...
//		int pkId = new Random().nextInt(100);
		...
		int pkId =  repository.saveMerchantPaymentRequest(entity); //TODO testing spring security, we commented temporary. 
//		Should not be committed to feature branch as well
		
		log.info("Repository response: {}", pkId);
		
		if(pkId == -1) { //duplicate transaction detected
			log.error("Failed to save merchant payment request, possible duplicate transaction for merchantTxnReference: {}", paymentRequest.getPayment().getMerchantTxnRef());
		...
		log.info("Payment request is valid, "
				+ "no duplicate transaction detected for merchantTxnReference: {}", 
				paymentRequest);
	}
